...
@time e_fci, d1_fci, d2_fci = TPSChem.pyscf_fci(ints,4,4)
...
#davidson = TPSChem.Davidson(A,max_iter=400, nroots=nr, tol=1e-5)
davidson = TPSChem.Davidson(Hmap,v0=v0,max_iter=100, nroots=nr, tol=1e-5)
Adiag = StringCI.compute_fock_diagonal(problem,mf.mo_energy, e_mf)
#TPSChem.solve(davidson)
TPSChem.solve(davidson, Adiag=Adiag)
#TPSChem.solve(davidson, Adiag=Diagonal(A))
