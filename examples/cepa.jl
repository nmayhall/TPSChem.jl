...
#e_fci, d1_fci, d2_fci = TPSChem.pyscf_fci(ints, na, nb, conv_tol=1e-10,max_cycle=100, nroots=1);
...
e_cmf, U, Da, Db  = TPSChem.cmf_oo(ints, clusters, init_fspace, rdm1a,rdm1b,
...
nroots = 1
ref_vector = TPSChem.TuckerState(clusters, p_spaces, q_spaces, na,nb)
#
# initialize with eye
TPSChem.set_vector!(ref_vector, Matrix(1.0I, length(ref_vector),nroots))

ref  = TPSChem.CompressedTuckerState(ref_vector, thresh=-1);

TPSChem.add_cmf_operators!(cluster_ops, cluster_bases, ints, Da, Db);

for i in 2:20
    thresh =  0.1^i
    #e_cepa, v_cepa = TPSChem.do_fois_cepa(ref, cluster_ops, clustered_ham, thresh_foi=thresh, max_iter=50, nbody=2);
    #e_pt2, v_pt2 =    TPSChem.do_fois_pt2(ref, cluster_ops, clustered_ham, thresh_foi=1e-5, max_iter=50, nbody=2, tol=1e-8);
    e_ci, v_ci =       TPSChem.do_fois_ci(ref, cluster_ops, clustered_ham, thresh_foi=thresh, max_iter=50, nbody=2, tol=1e-8);
    @printf(" CI  : %12.6f \n",(e_ci+ints.h0))
    #@printf(" PT2 : %12.6f \n",(e_pt2+ints.h0))
    #@printf("CEPA : %12.6f \n",(e_cepa+ints.h0))
    println(thresh)
...
#e_pt2, v_pt2 =    TPSChem.do_fois_pt2(ref, cluster_ops, clustered_ham, thresh_foi=1e-5, max_iter=50, nbody=2, tol=1e-8);
#e_ci, v_ci =       TPSChem.do_fois_ci(ref, cluster_ops, clustered_ham, thresh_foi=1e-5, max_iter=50, nbody=2, tol=1e-8);
