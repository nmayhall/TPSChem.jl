...
e_fci, d1_fci, d2_fci = TPSChem.pyscf_fci(ints,3,3)
...
TPSChem.pyscf_write_molden(mol,basis,mf.mo_coeff,filename="scf.molden")

# Cl = TPSChem.localize(mf.mo_coeff,"boys", mf)
# TPSChem.pyscf_write_molden(mol,"6-31g",Cl,filename="boys.molden")
C = mf.mo_coeff
rdm_mf = C[:,1:2] * C[:,1:2]'
...
println(" done.")
flush(stdout)
# ints = TPSChem.pyscf_build_ints(mf.mol,Cl);
# println(" done.")
# flush(stdout)
...
#TPSChem.cmf_ci(ints, clusters, init_fspace, rdm1)
e_cmf, U = TPSChem.cmf_oo(ints, clusters, init_fspace, rdm1, verbose=0, gconv=1e-6)
