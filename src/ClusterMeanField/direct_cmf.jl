...
function compute_cmf_energy(mol::Molecule, C::Matrix, rdm1s, rdm2s, clusters; verbose=0)
    ...
    for ci in clusters
        ints_i = pyscf_build_ints(mol, C[:,ci.orb_list], Dzero);
        e1[ci.idx] = compute_energy(0, ints_i.h1, ints_i.h2, rdm1s[ci.idx], rdm2s[ci.idx])
        ...
        for cj in clusters
            ...
            v_pqrs = pyscf_build_eri(mol,moi,moi,moj,moj) 
            v_psrq = pyscf_build_eri(mol,moi,moj,moj,moi) 
            tmp = 0
            @tensor begin
...
function cmf_oo(mol::Molecule, Cguess::Matrix, clusters::Vector{MOCluster}, fspace, dguess; 
    ...
    function g_analytic(kappa)
        ...
        h1 = C' * pyscf_build_1e(mol) 
        for ci in clusters
            grad_1 = grad[:,ci.orb_list]
            moi = C[:,ci.orb_list]
            h_1   = h1 * moi
            v_111 = pyscf_build_eri(mol,C,moi,moi,moi) 
            @tensor begin
            ...
            for cj in clusters
                ...
                moj = C[:,cj.orb_list]
                v_212 = pyscf_build_eri(mol,C,moj,moi,moj) 
                v_122 = pyscf_build_eri(mol,C,moi,moj,moj) 
                d1 = rdm1_dict[ci.idx]
                d2 = rdm1_dict[cj.idx]
