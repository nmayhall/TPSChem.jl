...
# Reshape into a ReshapedArray view (keeps following the parent buffer through resize!).
# Zero-size targets get a fresh empty Array instead: TensorOperations 4+/StridedViews
# cannot compute strides for zero-size reshaped views, and with no elements there is no
# aliasing to preserve.
reshape2(a, dims) = prod(dims) == 0 ? similar(a, dims) :
