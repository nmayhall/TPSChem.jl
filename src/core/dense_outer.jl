...
#={{{=#
    ...
    Hmap = TPSChem.LinOpMat{T}(matvec, dim, true)
...
function get_nbody_tucker_space(v::BSstate, p_spaces, q_spaces, na, nb; nroots=1, nbody=2)
    ...
    if nbody >= 1 
        for ci in clusters
            ...
            TPSChem.add!(ci_vector, TPSChem.BSstate(clusters, tmp_spaces, na, nb))
        end
    end
    if nbody >= 2 
        for ci in clusters
            for cj in clusters
                ...
                TPSChem.add!(ci_vector, TPSChem.BSstate(clusters, tmp_spaces, na, na))
            end
        end
    end
    if nbody >= 3 
