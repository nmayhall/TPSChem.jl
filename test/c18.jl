...
# get integrals
mf = TPSChem.pyscf_do_scf(mol, verbose=2)
C = mf.mo_coeff
nbas = size(C)[1]
    
TPSChem.pyscf_write_molden(mol, C, filename="hf.molden")
...
ints = TPSChem.pyscf_build_ints(mol, C[:,act_space], 2.0*Cocc*Cocc');
...
Cl = TPSChem.localize(Cact, "boys", mf)
# 
# use fiedler vector to reorder orbitals
h,j,k = TPSChem.pyscf_get_jk(mol, C[:,1:tot_na] * C[:,1:tot_na]')
Cl = TPSChem.fiedler_sort(Cl,k)

TPSChem.pyscf_write_molden(mol, Cl, filename="boys.molden")
TPSChem.pyscf_write_molden(mol, Cocc, filename="occ.molden")

n_core = 18
core_space = inactive_space[1:n_core]
sig_space = inactive_space[n_core+1:end]
Csig = TPSChem.localize(C[:,sig_space], "boys", mf)
Csig = TPSChem.fiedler_sort(Csig,(h+j-.5*k))
TPSChem.pyscf_write_molden(mol, Csig, filename="sig.molden")
