...
@testset "TDMs" begin
    ...
    cluster_bases = TPSChem.compute_cluster_eigenbasis(ints, clusters, verbose=0, max_roots=100) 

    #for cb in cluster_bases
    #    display(cb)
    #end

    cluster_ops = Vector{TPSChem.ClusterOps{Float64}}()
    for ci in clusters
        push!(cluster_ops, TPSChem.ClusterOps(ci)) 
    end
    ...
    for ci in clusters
        ...
        @time cluster_ops[ci.idx]["AB"], cluster_ops[ci.idx]["ba"], cluster_ops[ci.idx]["BA"], cluster_ops[ci.idx]["ab"] = TPSChem.tdm_AB(cb)
        @time cluster_ops[ci.idx]["AAa"], cluster_ops[ci.idx]["Aaa"] = TPSChem.tdm_AAa(cb,"alpha")
        @time cluster_ops[ci.idx]["BBb"], cluster_ops[ci.idx]["Bbb"] = TPSChem.tdm_AAa(cb,"beta")
        #@time cluster_ops[ci.idx]["ABa"], cluster_ops[ci.idx]["Aba"] = TPSChem.tdm_ABa(cb,"alpha")
        #@time cluster_ops[ci.idx]["ABb"], cluster_ops[ci.idx]["Bba"] = TPSChem.tdm_ABa(cb,"beta")
