...
@testset "EST" begin
    ...
    cb_cmf = TPSChem.compute_cluster_eigenbasis(ints, clusters, verbose=1, max_roots=5,rdm1a=d1.a, rdm1b=d1.b,init_fspace=init_fspace) 
    cb_est = TPSChem.compute_cluster_est_basis(ints, clusters, d1.a, d1.b, thresh_schmidt=1e-4, init_fspace=init_fspace)

    
    clustered_ham = TPSChem.extract_ClusteredTerms(ints, clusters)
    cluster_ops = TPSChem.compute_cluster_ops(cb_est, ints);

    ci_vector = TPSChem.TPSCIstate(clusters)
    TPSChem.expand_to_full_space!(ci_vector, cb_est, na, nb)
    ...
    H = TPSChem.build_full_H(ci_vector, cluster_ops, clustered_ham)
    display(size(H))
    #display(H)
    println()
