...
# Do TPS
M=100
cluster_bases = TPSChem.compute_cluster_eigenbasis_spin(ints, clusters, d1, [3,3,3], ref_fock, max_roots=M, verbose=1);
...
ept1 = TPSChem.compute_pt2_energy(vtpsci, cluster_ops, clustered_ham, thresh_foi=1e-12)
...
ept2 = TPSChem.compute_pt2_energy(vbst, cluster_ops, clustered_ham, thresh_foi=1e-64)
...
e_var, v = TPSChem.ci_solve(vbst, cluster_ops, clustered_ham);
s_bst = TPSChem.build_compressed_1st_order_state(v, cluster_ops, clustered_ham, nbody=4, thresh=1e-64)
TPSChem.pseudo_canon_pt1(s_bst, v, cluster_ops, clustered_ham, tol=1e-8, verbose=1);
